...
#+begin_src R
  ...
  times <- seq(tend * KY2D, 0, by = 0.2 * KY2D)
#+end_src
...
#+begin_src R :eval query
  print(system.time(
    out <- ode(y = state,
               ...
               func = eqns,
               parms = parameters,
               method = "ode23") # ode23 ode45 radau
  ))
#+end_src

#+RESULTS:
: user  system elapsed
: 361.986   0.213 364.353 # rk4, 1000 steps
: 334.800   0.081 336.341 # rk4, 1000 steps, no error
: 2927.945  187.784 3136.222 # ode23 5000 steps with interpolation of hh kk much slower
...
#+begin_src R
  ...
  write_rds(out, "out/2023-03-31_out.rds")
#+end_src
...
#+begin_src R
  ...
  ## out <- read_rds("out/2023-03-30_out.rds")
  out <- read_rds("out/2023-03-31_out.rds")
#+end_src
...
: 0.354433949865365 0.288908441936762 0.889073541112711
: -0.000228389007675633
