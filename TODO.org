...
** NEXT [1/4] make the La2011 solution obl + precession available? :prj:
:PROPERTIES:
...
*** NEXT ask Richard which corresponds to mean longitude
:PROPERTIES:
:CREATED:  [2023-04-10 Mon 16:55]
:END:

*** NEXT reverse engineer the parameters
:PROPERTIES:
...
** SOME [0/4] add options for which output to return
:LOGBOOK:
- State "SOME"       from              [2023-04-10 Mon 15:37]
:END:
- [ ] the raw spin vector output
- [ ] the full output with all columns
- [ ] only the same output as the C-routine (default)
...
* [29/39] snvecR is /published/ on CRAN, github, zenodo       :prj:
:PROPERTIES:
:CREATED:  [2023-04-04 Tue 09:27]
:END:
